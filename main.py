...
def get_user_reservations(
    ...
    reservations = (
        ...
        .filter(modals.Reservation.user_email == user_email)
...
def cancel_reservation(
    ...
    reservation = (
        db.query(modals.Reservation)
        .filter(modals.Reservation.id == reservation_id)
        .first()
    )
...
def dashboard(
    ...
    total_users = db.query(modals.User).count()
    total_orders = db.query(modals.Order).count()
    total_reservations = db.query(modals.Reservation).count()

    revenue = (
        db.query(modals.Order)
        .filter(modals.Order.payment_status == "Paid")
        .all()
    )

    total_revenue = sum(
        order.grand_total for order in revenue
    )
